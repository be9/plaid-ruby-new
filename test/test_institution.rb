...
class PlaidInstitutionTest < MiniTest::Test
  ...
  def test_string_representation
    ...
    str = %{#<Plaid::Institution id="5301a93ac140de84910000e0", type="bofa", name="Bank of America">}
  ...
  def test_all_institutions
    ...
    assert_equal({"username" => "User ID", "password" => "Password"}, i.credentials)
    assert i.has_mfa
    assert_equal ["code", "list"], i.mfa
  ...
  def test_get_nonexistent_category
    ...
    assert_equal 'Code 1301: unable to find institution. Double-check the provided institution ID.', e.message
  end

  private

  def bofa
    Plaid::Institution.new({
      "credentials" => {
        "username" => "Online ID",
        "password" => "Password"
      },
